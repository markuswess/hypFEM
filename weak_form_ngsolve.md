...
The terminology reflects the quadratic Helmholtz pencil. The factor $k^2m$ is a weighted mass form. The factor $ikc$ contains the first-order transport generated by the phase transformation together with the outflow term at infinity. We call $s$ the stiffness form, following standard finite-element terminology and the variable names used in the notebooks. Because it carries the Helmholtz sign and includes lower-order terms, this name does not imply that the form is positive definite.
