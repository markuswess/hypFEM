...
\frac{H}{\rho}u\,X\cdot\nabla\overline v\,dx
+\int_{\mathcal D}
\frac{H}{\rho}\overline v\,X\cdot\nabla u\,dx +\int_{\Gamma_\infty}u\overline v\,ds .
\end{aligned}
$$

The signed spatial form is
...
The terminology reflects the quadratic Helmholtz pencil. The factor $k^2m$ is a weighted mass form. The factor $ikc$ contains the first-order transport generated by the phase transformation together with the outflow term at infinity. The form $s$ is often called the Helmholtz stiffness form in code, but it is more precisely a *signed spatial form*: its leading gradient term has the Helmholtz sign, and the form is not being asserted to be positive definite.

The outgoing condition is encoded in the choice of space. For a standard compactification, an incoming branch has compactified radial derivatives of order $1/G$ and is not in $H^1(\mathcal D)$, whereas the transformed outgoing branch has a regular trace. This consistency statement should not be confused with a general coercivity or well-posedness theorem for every geometry and wavenumber.
...
| $s$ | compactified spatial metric and conformal weight | signed spatial form with anisotropic gradient and lower-order rescaling terms |
